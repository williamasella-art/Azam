...
export function Alarms() {
  ...
  const toggle = async (a: Alarm, enabled: boolean) => {
    try {
      if (enabled && a.repeat === 'once' && !nextFire(a)) { setModal({ type: 'alarm-form', alarm: a, title: 'Ubah alarm' }); notify('Waktu alarm ini sudah lewat. Pilih tanggal baru.'); return; }
      await saveAlarm({ ...a, enabled }, a.id); notify(enabled ? `Alarm ${showTime(a.time)} aktif.` : `Alarm ${showTime(a.time)} dimatikan.`);
    } catch (e: any) { notify(e.message); }
  };
  const remove = async (a: Alarm) => { try { await removeAlarm(a.id); notify('Alarm dihapus.'); } catch (e: any) { notify(e.message); } };
  ...
  return <Page title="Alarm dzikir" subtitle="Bangun dan berhenti sejenak dengan kalimat baik." back="focus"
    right={<Tap testID="alarm-add-button" style={s.addButton} onPress={() => setModal({ type: 'alarm-form' })} accessibilityLabel="Tambah alarm"><Icon name="add" size={24} color={colors.onBrandPrimary} /></Tap>}>
    <Card style={s.hero}><View style={s.heroArt}><AnimatedCat size={92} /></View><View style={{ flex: 1, gap: 4 }}>
      {next ? <><Badge text="ALARM BERIKUTNYA" icon="alarm-outline" /><T testID="alarms-next-time" size={34} weight="800" style={{ letterSpacing: -1.5 }}>{showTime(next.alarm.time)}</T><T size={12} weight="600">{next.alarm.label}</T><T testID="alarms-next-until" size={11} muted>{describeRepeat(next.alarm)} · dalam {untilText(next.at, now)}</T></>
        : <><Badge text="BELUM ADA ALARM AKTIF" icon="moon-outline" /><T size={16} weight="700">Atur alarm pertamamu</T><T size={11} muted>Tentukan tanggal, jam, dan dzikir yang ingin kamu ucapkan saat bangun.</T></>}
    </View></Card>
    {permission === 'missing' && <Tap testID="alarms-permission-banner" style={s.warn} onPress={() => setModal({ type: 'notifications' })}><Icon name="notifications-off-outline" size={20} color={colors.warning} /><T size={12} weight="600" color={colors.warning} style={{ flex: 1 }}>Izin notifikasi belum aktif, jadi alarm belum bisa berbunyi di HP ini. Ketuk untuk mengizinkan.</T><Icon name="chevron-forward" size={16} color={colors.warning} /></Tap>}
    {alarms.isLoading ? <Status loading /> : alarms.isError ? <Status error={alarms.error} retry={alarms.refetch} /> : !list.length ? <Card style={s.empty}><Icon name="alarm-outline" size={36} color={colors.muted} /><T size={15} weight="700" style={s.center}>Belum ada alarm</T><T size={12} muted style={s.center}>Tambahkan alarm untuk bangun subuh, tahajud, atau pengingat dzikir kapan pun.</T><Button testID="alarm-empty-add-button" title="Tambah alarm" icon="add-circle-outline" onPress={() => setModal({ type: 'alarm-form' })} /></Card>
      : list.map(a => { const at = a.enabled ? nextFire(a, now) : null; const slug = a.id.slice(-6); return <Card key={a.id} testID={`alarm-card-${slug}`} style={[s.card, !a.enabled && s.cardOff]}>
        <Tap testID={`alarm-edit-${slug}`} style={s.mainRow} onPress={() => setModal({ type: 'alarm-form', alarm: a, title: 'Ubah alarm' })}>
          <View style={{ flex: 1, gap: 2 }}><T testID={`alarm-time-${slug}`} size={40} weight="800" color={a.enabled ? colors.onSurface : colors.muted} style={{ letterSpacing: -1.5 }}>{showTime(a.time)}</T>
            ...
            <T size={11} muted>{at ? `Berbunyi dalam ${untilText(at, now)} · “${a.phrase}”` : a.enabled ? 'Waktunya sudah lewat' : 'Nonaktif'}</T></View>
        ...
        <View style={s.actions}>
          <Tap testID={`alarm-preview-${slug}`} style={s.action} onPress={() => setModal({ type: 'alarm', alarm: a, preview: true })}><Icon name="play-outline" size={16} color={colors.onBrandSecondary} /><T size={11} weight="700" color={colors.onBrandSecondary}>Coba bunyi</T></Tap>
          {Platform.OS === 'android' && <Tap testID={`alarm-clock-${slug}`} style={s.action} onPress={() => toClock(a)}><Icon name="phone-portrait-outline" size={16} color={colors.onBrandSecondary} /><T size={11} weight="700" color={colors.onBrandSecondary}>Jam HP</T></Tap>}
          <Tap testID={`alarm-delete-${slug}`} style={s.action} onPress={() => remove(a)}><Icon name="trash-outline" size={16} color={colors.error} /><T size={11} weight="700" color={colors.error}>Hapus</T></Tap>
        </View>
      </Card>; })}
    <Card style={s.info}><Icon name="information-circle-outline" size={20} color={colors.onBrandSecondary} /><T size={11} muted style={{ flex: 1 }}>{Platform.OS === 'web' ? 'Di pratinjau web, alarm hanya tersimpan. Buka Azam lewat Expo Go atau build HP agar alarm benar-benar berbunyi dengan suara dan getar.' : Platform.OS === 'android' ? 'Alarm berbunyi sebagai notifikasi bersuara dan bergetar walau Azam ditutup. “Jam HP” juga menyetelnya di aplikasi Jam bawaan agar berbunyi maksimal.' : 'Alarm berbunyi sebagai notifikasi bersuara dan bergetar walau Azam ditutup. Pastikan mode Fokus/Senyap mengizinkan notifikasi Azam.'}</T></Card>
  </Page>;
}
