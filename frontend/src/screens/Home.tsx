      ...
      <Animated.View entering={FadeInDown.duration(500)} style={s.contentInner}>
        ...
        <ImageBackground source={IMG.heroBirds} style={s.hero} imageStyle={s.heroImage} testID="next-prayer-card" onLayout={e => setHeroSize({ w: e.nativeEvent.layout.width, h: e.nativeEvent.layout.height })}>
          ...
          <View style={s.heroContent}><View style={s.heroEyebrow}><View style={s.dot} /><T size={10} weight="700" color={colors.heroMuted}>{t('home.nextPrayer')}{isTomorrow ? ` · ${t('home.tomorrow')}` : ''}</T></View>
            ...
            <View style={s.heroFooter}><Icon name="location-outline" size={12} color={colors.heroMuted} /><T size={10} color={colors.heroMuted}>{settings.city} · {t('home.byLocation')}</T></View>
          </View>
        </ImageBackground>
        ...
        <Tap testID="home-alarm-button" onPress={() => go('alarms')} style={s.blockerCard}><IconBox name="alarm" size={56} icon={26} bg={colors.brandPrimary} color={colors.onBrandPrimary} /><View style={{ flex: 1, gap: 4 }}><Badge text={nextAlarm ? t('home.nextAlarm') : t('home.dzikirAlarm')} icon="alarm-outline" /><T weight="700" size={14}>{nextAlarm ? `${showTime(nextAlarm.alarm.time)} · ${nextAlarm.alarm.label}` : t('home.wakeDzikir')}</T><T size={11} muted>{nextAlarm ? `${describeRepeat(nextAlarm.alarm)} · ${t('home.in')} ${untilText(nextAlarm.at, now)}` : t('home.alarmHint')}</T></View><Icon name="chevron-forward" size={18} color={colors.muted} /></Tap>
