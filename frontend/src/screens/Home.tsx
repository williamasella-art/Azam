  ...
  return <Bg>
    ...
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.content}>
      <Animated.View entering={FadeInDown.duration(500)} style={s.contentInner}>
        ...
        {nextEvent && <Tap testID="home-event-button" onPress={() => go('progress')} style={s.eventPill}><Icon name={nextEvent.icon} size={18} color={colors.goldText} /><View style={{ flex: 1 }}><T size={12} weight="700">{nextEvent.name}</T><T size={10} muted>{nextEvent.hijri} · {new Date(`${nextEvent.date}T12:00:00`).toLocaleDateString('id-ID', { day: 'numeric', month: 'long' })}</T></View><Badge text={nextEvent.days_until === 0 ? 'HARI INI' : `${nextEvent.days_until} HARI LAGI`} gold /></Tap>}
        <Tap testID="home-alarm-button" onPress={() => go('alarms')} style={s.blockerCard}><IconBox name="alarm" size={56} icon={26} bg={colors.brandPrimary} color={colors.onBrandPrimary} /><View style={{ flex: 1, gap: 4 }}><Badge text={nextAlarm ? 'ALARM BERIKUTNYA' : 'ALARM DZIKIR'} icon="alarm-outline" /><T weight="700" size={14}>{nextAlarm ? `${showTime(nextAlarm.alarm.time)} · ${nextAlarm.alarm.label}` : 'Bangun dengan dzikir'}</T><T size={11} muted>{nextAlarm ? `${describeRepeat(nextAlarm.alarm)} · dalam ${untilText(nextAlarm.at, now)}` : 'Atur tanggal & jam, alarm berbunyi di HP-mu'}</T></View><Icon name="chevron-forward" size={18} color={colors.muted} /></Tap>
        ...
        <Tap testID="home-pro-banner" onPress={() => go('pro')} style={s.proBanner}><ImageBackground source={IMG.hajj} style={s.proBg} imageStyle={{ borderRadius: 24 }}><LinearGradient colors={[colors.heroShade, colors.overlay, colors.transparent]} start={{ x: 0, y: 0.5 }} end={{ x: 1, y: 0.5 }} style={s.heroShade} /><View style={s.proBody}><Badge text="AZAM PRO" gold icon="sparkles" style={{ backgroundColor: colors.gold }} /><T size={17} weight="800" color={colors.heroInk}>Haji & Umroh, suara premium,{"\n"}tema malam & widget</T><View style={s.proCta}><T size={11} weight="700" color={colors.goldInk}>{settings.pro_preview ? 'Pratinjau aktif · Jelajahi' : 'Coba pratinjau gratis'}</T><Icon name="arrow-forward" size={13} color={colors.goldInk} /></View></View></ImageBackground></Tap>
