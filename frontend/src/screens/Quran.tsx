...
function ReaderContent({ data }: { data: any }) {
  const { settings, updateSettings, surah, notify, setModal } = useApp(); const { active, toggle } = useAmbient(); const s = useStyles(); const { colors } = useTheme();
  ...
  return <>
    <View style={s.readerControls}><View style={s.switchLabel}><T size={11} muted>Terjemahan</T><Switch testID="reader-translation-switch" value={settings.translation} onValueChange={(value) => updateSettings({ translation: value })} trackColor={{ false: colors.borderStrong, true: colors.brandPrimary }} thumbColor={colors.white} /></View>
      <View style={{ flex: 1 }} />
      <Tap testID="reader-rain-button" style={[s.audioButton, active === 'rain' && s.audioActive]} onPress={() => toggle('rain')}><Icon name="rainy-outline" size={18} color={active === 'rain' ? colors.onBrandPrimary : colors.onSurfaceTertiary} /></Tap>
      <Tap testID="reader-cat-button" style={[s.audioButton, active === 'cat' && s.audioActive]} onPress={() => toggle('cat')}><Icon name="paw-outline" size={18} color={active === 'cat' ? colors.onBrandPrimary : colors.onSurfaceTertiary} /></Tap>
    ...
    <FlatList ref={listRef} testID="reader-verse-list" data={data.ayat} keyExtractor={item => String(item.nomorAyat)} showsVerticalScrollIndicator={false} contentContainerStyle={s.list}
      ...
      renderItem={({ item }) => {
        ...
        return <Paper testID={`verse-${item.nomorAyat}`} style={s.verse}><View style={s.verseTop}><View style={s.verseNumber}><T size={11} color={colors.brandDeep} weight="800">{surah}:{item.nomorAyat}</T></View><View style={s.verseActions}><Tap testID={`verse-${item.nomorAyat}-share-button`} style={s.smallButton} onPress={() => setModal({ type: 'share-verse', verse: { ...item, surah: data.namaLatin, number: surah } })}><Icon name="share-social-outline" size={18} color={colors.paperMuted} /></Tap><Tap testID={`verse-${item.nomorAyat}-bookmark-button`} style={s.smallButton} onPress={async () => { if (await updateSettings({ last_surah: surah, last_verse: item.nomorAyat })) notify('Penanda bacaan disimpan.'); }}><Icon name={bookmarked ? 'bookmark' : 'bookmark-outline'} size={19} color={bookmarked ? colors.brandDeep : colors.paperMuted} /></Tap></View></View>
          <T arabic paper size={27} testID={`verse-${item.nomorAyat}-arabic`} style={s.arabic}>{item.teksArab}</T>{settings.translation && <T paper muted testID={`verse-${item.nomorAyat}-translation`} size={13} style={s.translation}>{item.teksIndonesia}</T>}
        </Paper>;
...
const useStyles = makeStyles(c => ({
  ...
  readerHero: { alignItems: 'center', gap: 5, marginBottom: 6, paddingVertical: 22 }, verse: { gap: 12, padding: 16 }, verseTop: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, verseNumber: { backgroundColor: c.paperTint, borderRadius: 9, paddingHorizontal: 11, paddingVertical: 6 }, verseActions: { flexDirection: 'row', gap: 2 }, smallButton: { width: 44, height: 44, justifyContent: 'center', alignItems: 'center' }, arabic: { writingDirection: 'rtl', textAlign: 'right', lineHeight: 54 }, translation: { lineHeight: 22 },
}));
