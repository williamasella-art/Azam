...
export function Progress() {
  const { progress, go, day, month, setMonth, setModal, events } = useApp(); const s = useStyles(); const { colors } = useTheme();
  const data = progress.data;
  const eventList: any[] = events.data || []; const eventByDate: Record<string, any> = {}; eventList.forEach(e => { eventByDate[e.date] = eventByDate[e.date] || e; });
  const upcoming = eventList.filter(e => e.days_until >= 0).slice(0, 6);
    ...
    {progress.isLoading || progress.error ? <Status loading={progress.isLoading} error={progress.error} retry={progress.refetch} /> : <>
      ...
      <Paper style={s.calendar}><View style={s.monthRow}><View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}><Icon name="calendar" size={18} color={colors.brandDeep} /><T paper testID="calendar-month-label" weight="700" size={15}>{new Date(year, monthNum - 1, 1).toLocaleDateString('id-ID', { month: 'long', year: 'numeric' })}</T></View><View style={{ flexDirection: 'row' }}><Tap testID="calendar-previous-button" style={s.calendarNav} onPress={() => move(-1)}><Icon name="chevron-back" size={18} color={colors.onPaper} /></Tap><Tap testID="calendar-next-button" style={s.calendarNav} onPress={() => move(1)}><Icon name="chevron-forward" size={18} color={colors.onPaper} /></Tap></View></View>
        ...
        <View style={s.calendarGrid}>{Array.from({ length: first + length }, (_, i) => {
          const n = i - first + 1; const date = `${month}-${String(n).padStart(2, '0')}`; const count = data.calendar[date]?.length || 0; const event = eventByDate[date];
          if (n < 1) return <View key={i} style={s.dayCell} />;
          return <Tap testID={`calendar-day-${n}`} key={i} onPress={() => date > day ? (event ? setModal({ type: 'info', title: event.name, message: `${event.hijri} · ${new Date(`${date}T12:00:00`).toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}\n\n${event.description}` }) : null) : setModal({ type: 'day', date })} style={[s.dayCell, date === day && s.today, count === 5 && s.fullDay, event && date !== day && count !== 5 && s.eventDay]}><T size={12} weight={date === day || event ? '800' : '500'} color={count === 5 ? colors.white : event ? colors.goldText : date > day ? colors.paperMuted : colors.onPaper}>{n}</T>{event ? <Icon name="star" size={7} color={count === 5 ? colors.white : colors.goldText} /> : <View style={[s.dayDot, { backgroundColor: count === 5 ? colors.white : count ? colors.brandDeep : colors.paperBorder }]} />}</Tap>;
        })}</View>
        <View style={s.legend}>{[[colors.brandDeep, 'Lengkap'], [colors.brandTertiary, 'Sebagian'], [colors.paperBorder, 'Belum']].map(([color, label]) => <View key={label} style={s.legendItem}><View style={[s.dayDot, { backgroundColor: color }]} /><T paper muted size={9}>{label}</T></View>)}<View style={s.legendItem}><Icon name="star" size={8} color={colors.goldText} /><T paper muted size={9}>Hari besar Islam</T></View></View>
        {eventList.filter(e => e.date.startsWith(month)).map(e => <View key={e.key} testID={`calendar-event-${e.key}`} style={s.monthEvent}><Icon name={e.icon} size={16} color={colors.goldText} /><View style={{ flex: 1 }}><T paper size={12} weight="700">{e.name}</T><T paper muted size={10}>{new Date(`${e.date}T12:00:00`).toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long' })} · {e.hijri}</T></View></View>)}
      </Paper>
      <Section title="Hari besar Islam mendatang" />
      {events.isLoading ? <Status loading /> : <Card style={s.eventCard} testID="islamic-events-card">{upcoming.length ? upcoming.map((e, i) => <View key={e.key} testID={`islamic-event-${i}`} style={[s.eventRow, i === upcoming.length - 1 && { borderBottomWidth: 0 }]}><View style={s.eventIcon}><Icon name={e.icon} size={20} color={colors.goldText} /></View><View style={{ flex: 1, gap: 2 }}><T size={14} weight="700">{e.name}</T><T size={11} muted>{new Date(`${e.date}T12:00:00`).toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })} · {e.hijri}</T><T size={11} muted numberOfLines={2}>{e.description}</T></View><View style={s.countdown}><T size={16} weight="800" color={colors.goldText}>{e.days_until === 0 ? 'Hari' : e.days_until}</T><T size={9} color={colors.goldText}>{e.days_until === 0 ? 'ini' : 'hari lagi'}</T></View></View>) : <T size={12} muted>Jadwal hari besar belum tersedia.</T>}<T size={9} muted>Tanggal mengikuti kalender Umm al-Qura; penetapan resmi dapat berbeda satu hari sesuai rukyat pemerintah.</T></Card>}
...
const useStyles = makeStyles(c => ({
  ...
  eventDay: { backgroundColor: c.goldSoft }, monthEvent: { flexDirection: 'row', alignItems: 'center', gap: 10, paddingTop: 10, borderTopWidth: 1, borderTopColor: c.paperBorder },
  eventCard: { gap: 4, padding: 14 }, eventRow: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: c.divider }, eventIcon: { width: 42, height: 42, borderRadius: 14, backgroundColor: c.goldSoft, alignItems: 'center', justifyContent: 'center' }, countdown: { alignItems: 'center', minWidth: 52, paddingVertical: 6, paddingHorizontal: 8, borderRadius: 12, backgroundColor: c.goldSoft },
