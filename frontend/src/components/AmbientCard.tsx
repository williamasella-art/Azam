...
export function AmbientCard({ compact = false }: { compact?: boolean }) {
  ...
  return <Card style={s.card} testID="ambient-card">
    {!compact && <View style={s.head}><View style={{ flex: 1, gap: 4 }}><Badge text="RAMAH ADHD · FOKUS" icon="sparkles-outline" /><T size={17} weight="800">Suasana tenang</T><T size={11} muted>Atur volume tiap suara sesuai kenyamananmu.</T></View>{active === 'rain' ? <Image source={IMG.rain} style={s.art} /> : <View style={s.catArt}><AnimatedCat size={90} sleepy={active !== 'cat'} /></View>}</View>}
