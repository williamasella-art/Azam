...
/** Interactive 9:16 story composer: background, tint, caption, stats, then share as image (native) or text (web). */
    ...
    <ViewShot ref={shot} options={{ format: 'png', quality: 1 }} style={s.shotWrap}>
      <Animated.View key={`${template}-${tint}`} entering={ZoomIn.duration(320)}>
        <ImageBackground source={TEMPLATES[template].image} style={[s.card, { width: cardWidth, height: cardHeight }]} imageStyle={{ borderRadius: 28 }} testID="share-template">
          ...
          <View style={s.cardTop}><Logo size={28} wordmark /><Badge text={isVerse ? 'SEAYAT' : completed ? 'SALAT TERCATAT' : isBadge ? `LENCANA ${badge.name.toUpperCase()}` : `LEVEL ${(level?.name || 'PEMULA').toUpperCase()}`} gold style={{ backgroundColor: colors.overlay }} /></View>
          {isVerse ? <View style={s.verseBox}><T arabic size={26} color={colors.heroInk} style={s.center}>{verse.teksArab}</T><T size={13} color={colors.heroMuted} style={s.center}>“{verse.teksIndonesia}”</T><Badge text={`QS. ${verse.surah} : ${verse.nomorAyat}`} gold /></View>
            : isBadge ? <View style={s.streakBox} testID="share-badge-box">
              <LevelBadge name={badge.name} size={172} />
              <T size={12} weight="700" color={colors.goldText} style={{ letterSpacing: 1.2 }}>MASYA ALLAH</T>
              <T testID="share-badge-title" size={34} weight="800" color={colors.heroInk} style={{ letterSpacing: -1, lineHeight: 40 }}>Level {badge.name}</T>
              <T size={13} color={colors.heroMuted} style={[s.center, { paddingHorizontal: 8, lineHeight: 20 }]}>{LEVEL_COPY[badge.name]}</T>
            ...
            : <View style={s.streakBox}>
              <LevelBadge name={completed ? 'Bintang' : level?.name || 'Awan'} size={150} locked={!completed && !level} />
...
const useStyles = makeStyles(c => ({
  ...
  card: { borderRadius: 28, overflow: 'hidden', padding: 22, justifyContent: 'space-between' }, shade: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, opacity: 0.85 },
  cardTop: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }, cardBottom: { alignItems: 'center', gap: 6 },
  verseBox: { gap: 16, alignItems: 'center', paddingVertical: 12 }, streakBox: { alignItems: 'center', gap: 8 }, streakRow: { flexDirection: 'row', alignItems: 'center', gap: 8 },
